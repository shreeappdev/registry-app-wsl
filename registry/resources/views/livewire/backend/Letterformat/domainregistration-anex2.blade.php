<html>
	...
	<body>            
		<div class="letterfwd">
			...
			<p><strong>Subject:</strong> "Regarding registration of domain name {{$domainDetails->dname_decoded_punycode}} @if(!empty( $domainDetails->idnDomainDetails) && !empty($domainDetails->idnDomainDetails->domainname_decoded)) ({{ $domainDetails->idnDomainDetails->domainname_decoded }}) @endif ".</p>
			<p>I am hereby forwarding the Authorization Letter for the domain '{{$domainDetails->dname_decoded_punycode}} @if(!empty( $domainDetails->idnDomainDetails) && !empty($domainDetails->idnDomainDetails->domainname_decoded)) ({{ $domainDetails->idnDomainDetails->domainname_decoded }}) @endif' and  confirm that all information furnished in the Authorization Letter is correct to the best of my knowledge.</p>
			<p>I <i>{{$nodal_name}},{{$nodal_designation}}</i>  <strong>{{$organisationName}}</strong> endorse that the <strong>{{$organisationName}}</strong> is a government organization and belongs to the category <b>{{$orgcategory}}</b> of the guidelines vide no F.No. 13/14/2014-IGD dated 23<sup>th</sup> October 2019.  The domain name '{{$domain_name}} {{$idn_domain_name}}' would be used for official purposes and would conform to the IT Act of India and Aadhaar Act, 2016. Domain name will not be used for any unlawful & commercial purpose and as per MHA OM, the website will be hosted in India only.</p>
			...
			<p>Hence, I recommend {{$domain_name}} {{$idn_domain_name}} for <strong>{{$organisationName}}</strong>.</p><p><strong><u>Signing Authority</u></strong></p>
				...
				<table class="fwd">
						...
						<tr>
							...
							<td> {{ $nodal_region == 'state' ? 'State: ' . $state : 'Ministry: ' . $nodal_ministry }} </td>
							{{-- <td>Ministry: <i>{{$nodal_ministry}}</i></td> --}}
							<td>Region: <i>{{$nodal_region}}</i></td>	
						</tr>
						<tr>
							@if($nodal_region == 'central') <td>Department: <i>{{$nodal_dept}}</i></td> @endif
